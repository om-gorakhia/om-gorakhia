# Hi, I'm Om Gorakhia 👋 🚀
**Business Analyst | Data Scientist | ML Engineer**

I'm passionate about turning messy data into clear business insights. Currently pursuing my Master's in Business Analytics at NUS, I love building predictive models, automating workflows, and solving real-world business problems with data.

## 💼 What I Do

I help businesses make smarter decisions through data. My work focuses on:

- **Predictive Analytics** – Forecasting sales, customer behavior, and business metrics using machine learning
- **Data Pipeline Automation** – Building ETL systems and no-code platforms to streamline data workflows
- **NLP & AI Solutions** – Creating intelligent systems with LLMs, sentiment analysis, and text classification
- **Business Intelligence** – Designing dashboards and executive reports that translate complex data into actionable strategies

## 🛠️ Some Projects I'm Proud Of

- Built a **lead qualification platform** using LLMs that improved conversion rates by 40% for enterprise clients
- Created **predictive models for e-commerce sales** forecasting across Shopify, WooCommerce, and BigCommerce
- Developed an **industry classification system** using NLP with 94% accuracy across massive datasets
- Designed **customer segmentation algorithms** identifying 45+ distinct segments for personalized marketing

## 🔧 Tools I Work With

**Programming & Development:**
Python, R, SQL, PostgreSQL, JavaScript, JSON Processing, REST APIs, FastAPI, Streamlit, ETL Architectures, Git/GitHub

**Data Science & AI:**
Large Language Models (LLMs), Multi-Agent AI Orchestration, Agentic AI Architectures, Natural Language Processing (NLP), Prompt Engineering, Hugging Face, Sentence Transformers, Semantic Analysis, Transfer Learning, Sentiment Analysis, TensorFlow, Generative AI (GenAI)

**Machine Learning & Modeling:**
Statistical Modeling, Predictive Analytics, Regression Analysis (OLS, 2SLS, Panel/Fixed-Effects), L1/L2 Regularization, XGBoost, K-Means Clustering, A/B Testing, Hypothesis Testing, Probabilistic Scoring, Data Mining, Data Segmentation, SPSS

**Quantitative Finance:**
Markowitz Mean-Variance Optimization, Kelly Criterion, Value-at-Risk (VaR), Expected Shortfall, Sharpe / Sortino / Calmar Ratios, SciPy Linear Programming, Covariance Shrinkage, Backtesting (LOOCV), Technical Indicators (EMA, RSI, ATR, Stochastic)

**Business Intelligence & Visualization:**
Power BI, Microsoft Excel, Plotly, Data Dashboards, Wireframes, UML Diagrams, Executive Reporting

**Platform & Automation:**
N8N, Microsoft Power Apps, Microsoft Power Automate, No-Code Development, Data Pipeline Automation, Workflow Optimization, Design Thinking

**Languages:**
English, Hindi

## 📫 Let's Connect!

- **GitHub:** You're already here! 🎉
- **LinkedIn:** [linkedin.com/in/omgorakhia](https://linkedin.com/in/omgorakhia)
- **Email:** om.gorakhia@u.nus.edu

I'm always open to interesting conversations about data, analytics, and building tools that actually help people make better decisions. Feel free to reach out!

---
*Currently based in Singapore 🇸🇬 | Previously helped companies like Amazon, Walmart, and PayPal with data intelligence solutions*
